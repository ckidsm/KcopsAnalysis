...
namespace KcopsAnalysis
{
    public partial class FormOpenVideo : Form
    {
        ...
        //알고리즘 디렉토리 
        private readonly string AlgorithmDirectory = @"C:\HitRun\code_1205";
        ...
        private void iconBtnAnalyze_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (File.Exists(outputinfo.OutputImage))
                {
                    ...
                    TextWriter.LoggingToFile(GetType().Name, $"영상 분석 클릭 :: isRunning 상태: {isRunning} :: 충격 수치 파일 삭제 {outputinfo.OutputImage} ");
                }
        ...
        private async Task<string> ProcessStartAsync()
        {
            ...
            try
            {
                ...
                string AlgorithmArgumentsModeNew = "python main.py --video " + sourceInfo.FileFullName;

                StstusPrint($" 명령어/ 소스 경로/ 옵션 => {AlgorithmArgumentsModeNew}");
